...
-- dns packages
import DNS.Types
  (TYPE(NS, A, AAAA), ResourceRecord (ResourceRecord, rrname, rrtype, rdata), rd_ns, rd_a, rd_aaaa)
...
rootServers =
  ...
  where
    mkRR n ty tt rd = ResourceRecord { rrname = n, rrtype = ty, DNS.rrclass = DNS.classIN, DNS.rrttl = tt, rdata = rd }
