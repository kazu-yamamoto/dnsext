...
descs =
  ...
  , Option [] ["per-worker"]
    (ReqArg (\s opts -> readIntWith (>= 0) "per-worker. negative not allowed" s >>= \x -> return opts { qsizePerWorker = x }) "POSITIVE_INTEGER")
    "queue size per worker. default is 16. positive integer or 0. 0 means not limited size queue"
  , Option ['r'] ["requests"]
    (ReqArg (\s opts -> readIntWith (> 0) "requests. not positive" s >>= \x -> return opts { requests = x }) "POSITIVE_INTEGER")
