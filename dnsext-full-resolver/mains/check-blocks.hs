...
import Data.IP
  (IP (IPv4, IPv6), fromIPv4, fromIPv6b,
   IPRange (IPv4Range, IPv6Range), AddrRange (addr, mlen))
import Network.DNS
  (ResolvConf (..), ResolvSeed, TYPE (PTR),
   DNSError, DNSMessage)
...
ipRevDomain ip = case ip of
  ...
  where
    ...
    rev6 v6 =
      concat [ showHex h [] ++ "." | byte <- reverse $ fromIPv6b v6, h <- e2fRev byte ]
      ++ "ip6.arpa."
      where e2fRev byte = [byte .&. 0x0f, byte `shiftR` 4 ]
...
rdataPair rd = (codeFromRData rd, show rd)
  where
    ...
    codeFromRData (DNS.RD_SOA dom _ _ _ _ _ _)
      ...
      | otherwise                               =  "0"
    codeFromRData  _                            =  "2"
    knownDomains = map B8.pack ["arin.net.", "in-addr-servers.arpa.", "ip6-servers.arpa.", "6to4.nro.net."]
