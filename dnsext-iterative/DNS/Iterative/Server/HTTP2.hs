...
http2Server VcServerConfig{..} env toCacher s = do
    name <- socketName s
    let http2server = withLocationIOE name $ H2TLS.runIOWithListenSocket settings vc_credentials s $ doHTTP "h2" sbracket incQuery env toCacher
    return [http2server]
  where
...
http2cServer VcServerConfig{..} env toCacher s = do
    name <- socketName s
    let http2server = withLocationIOE name $ H2TLS.runIOH2CWithListenSocket settings s $ doHTTP "h2c" sbracket incQuery env toCacher
    return [http2server]
  where
