...
eddsaDecodePubKey :: String -> (ByteString -> CryptoFailable pubkey) -> PubKey -> Either String pubkey
eddsaDecodePubKey algName consPublicKey (PubKey_Opaque ks) =
  eitherCryptoFailable (algName ++ ".publicKey") . consPublicKey $ Opaque.toByteString ks
eddsaDecodePubKey _       _             _           = do
  Left "eddsaDecodePubKey: not EdDSA pubkey format"

eddsaDecodeSignature :: String -> (ByteString -> CryptoFailable sig) ->  Opaque -> Either String sig
eddsaDecodeSignature algName consSignature = eitherCryptoFailable (algName ++ ".signature") . consSignature . Opaque.toByteString

eddsaVerify :: (pubkey -> ByteString -> sig -> Bool) -> pubkey -> sig -> ByteString -> Either String Bool
