...
toPubKey_RSA o = PubKey_RSA len e n
  where
    ...
    divide elen o' = let (e',n') = Opaque.splitAt elen o'
...
toPubKey_ECDSA len o
  | len * 2 == blen = let (x,y) = Opaque.splitAt len o
                      in PubKey_ECDSA x y
...
fromPubKey (PubKey_RSA len e n)
    | len >= 256 = let (x,y) = len `divMod` 256
    ...
    | otherwise  = Opaque.concat [ Opaque.singleton $ fromIntegral len
                                 , e
                                 , n
                                 ]
